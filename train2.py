...
from read_union import load_dataset2, read_dataset_part_ids, verify_union
...
def load_checkpoint(model: TinyGPT, optimizer: torch.optim.Optimizer, device: torch.device) -> tuple[int, int, int, list[float]]:
    ...
    print(
        f"loaded {MODEL_PATH}: epoch={epoch} epoch_step={epoch_step} global_step={global_step}",
        flush=True,
    )
...
def train_epoch(
    ...
    for step in range(micro_start, steps_per_epoch * accumulation):
        ...
        weighted_loss = dataset_loss * DATASET_LOSS_WEIGHT + dataset2_loss * DATASET2_LOSS_WEIGHT
...
def main() -> None:
    ...
    if not dataset_id_samples or len(dataset2) == 0:
        raise RuntimeError("No training data found from read_union.py ids.")

    print("building dataset1 tensor and dataset2 sample index ...", flush=True)
    ...
    stats = {
        "files": int(dataset_stats["files"]) + int(dataset2_stats["files"]),
        "tokens": int(dataset_stats["tokens"]) + int(dataset2_stats["tokens"]),
        "longest_context": max(int(dataset_stats["longest_context"]), int(dataset2_stats["longest_context"])),
        "token_memory_mb": round(float(dataset_stats["token_memory_mb"]) + float(dataset2_stats["token_memory_mb"]), 3),
    }
    ...
    if result_meets_targets() and not args.force:
        ...
        for epoch in range(first_epoch, args.max_epochs + 1):
            ...
            token_acc = dataset_acc * DATASET_LOSS_WEIGHT + dataset2_acc * DATASET2_LOSS_WEIGHT
            ppl = dataset_ppl * DATASET_LOSS_WEIGHT + dataset2_ppl * DATASET2_LOSS_WEIGHT
            result = build_result(
                token_acc,
                ppl,
                dataset_acc,
